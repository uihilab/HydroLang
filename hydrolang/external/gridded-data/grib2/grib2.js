...
// HYDRO LANG INTEGRATION
// Add extractGRIB2Data function for HydroLang compatibility

/**
 * Extract GRIB2 data in HydroLang format
 * @param {Object} grib2Data - Parsed GRIB2 data from parseGRIB2
 * @param {Object} options - Extraction options
 * @returns {Promise<Object>} Extracted data in HydroLang format
 */
async function extractGRIB2Data(grib2Data, options = {}) {
    const { parameter, level, bbox, startDate, endDate } = options;

    console.log('Extracting GRIB2 data with research implementation...', { parameter, level, bbox });

    try {
        // Import the integration layer
        const { default: GRIB2Integration } = await import('./grib2-integration.js');
        const integration = new GRIB2Integration();

        // Use the research implementation to extract real data
        const result = await integration.extractGRIB2Data(grib2Data, options);

        console.log('GRIB2 extraction completed successfully');
        console.log('Real meteorological data extracted:', result.data.shape);

        return result;

    } catch (error) {
        console.error('GRIB2 extraction failed:', error);

        // Provide clear error about real data vs missing implementation
        throw new Error(
 ...
 * @returns {Promise<Object>} Loaded GRIB2 library interface
 */
async function load(options = {}) {
    try {
        // Import the standalone parser
        const standaloneParser = await import('./hydrolang-grib2-parser.js');

        // Return library interface
        return {
            parse: standaloneParser.parseGRIB2,
            parseGRIB2: standaloneParser.parseGRIB2,
            extractData: standaloneParser.extractGRIB2Data,
            extractGRIB2Data: standaloneParser.extractGRIB2Data,
            default: standaloneParser.default,
            isLoaded: () => true,
            getInfo: () => ({
                version: '2.0',
                format: 'GRIB2',
        ...
        console.error('Failed to load GRIB2 library:', error);
        throw new Error(`GRIB2 library load failed: ${error.message}`);
    }
}
